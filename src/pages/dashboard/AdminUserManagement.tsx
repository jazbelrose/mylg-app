...
export default function AdminUserManagement() {
  const {
    ...
    allUsers: UserLite[];
    refreshUsers: () => Promise<any> | void;
    isAdmin: boolean;
    projects: ProjectLite[];
    fetchProjects: () => Promise<any> | void;
  } = useData();
  ...
  const handleChange = (userId: string, field: keyof EditValuesMap[string], value: string | boolean) => {
    ...
    setEditValues((prev) => ({
      ...prev,
      [userId]: { ...prev[userId], [field]: normalized as any },
    }));
  };
  ...
  const toggleCollaboratorSelection = (userId: string, collaboratorId: string) => {
    setAssignedCollaborators((prev) => {
      const current = new Set(prev[userId] || []);
      current.has(collaboratorId) ? current.delete(collaboratorId) : current.add(collaboratorId);
      return { ...prev, [userId]: Array.from(current) };
    });
  };
