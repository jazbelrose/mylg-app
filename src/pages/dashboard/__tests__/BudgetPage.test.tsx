...
// Pull mocked modules with types
const { useData } = require("../../../app/contexts/DataProvider") as { useData: jest.Mock };
const { useSocket } = require("../../../app/contexts/SocketContext") as { useSocket: jest.Mock };
const api = require("../../../utils/api") as {
  fetchBudgetHeaders: jest.Mock;
  fetchBudgetItems: jest.Mock;
  createBudgetItem: jest.Mock;
  updateBudgetItem: jest.Mock;
};
const { useParams, useNavigate, useLocation } = require("react-router-dom") as {
  useParams: jest.Mock;
  useNavigate: jest.Mock;
  useLocation: jest.Mock;
};
const mockUseBudgetData = require("../components/SingleProject/useBudgetData").default as jest.Mock;

// SUT (required after mocks are configured)
...
beforeEach(() => {
  ...
  // Ensure a root (in case components use ReactModal/setAppElement)
  ...
  // Require SUT fresh each test so mocks apply
  BudgetPage = require("../BudgetPage").default;
});
...
test("clicking row opens edit modal with existing data", async () => {
  ...
  const firstRow = screen.getByText("Item 1").closest("tr") as HTMLTableRowElement;
...
test("cmd/ctrl+enter submits the create modal", async () => {
  ...
  const categorySelect = await screen.findByLabelText("Category", { selector: "select" });
  await userEvent.selectOptions(categorySelect, "DECOR");
...
test("revision buttons trigger createBudgetItem", async () => {
  ...
  await waitFor(() =>
    expect(api.createBudgetItem).toHaveBeenCalledWith(
      "p1",
      "b1",
