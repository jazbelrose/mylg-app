...
const NewProjectFinishline: React.FC<NewProjectFinishLineProps> = ({ finishline, setFinishLine }) => {
  ...
  const handleFinishLineClick = () => setShowFinishLineModal(true);
  const closeFinishLineModal = () => setShowFinishLineModal(false);

  const handleFinishLineChange = (e: ChangeEvent<HTMLInputElement>) => {
    setFinishLine(e.target.value);
  };

  const handleSubmitFinishLine = (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
  ...
  return (
    <>
      <div
        className="dashboard-item new-project-finish-line"
