...
const AllProjectsCalendar = () => {
  ...
  const projectsWithLanes = useMemo(() => {
    ...
    const sorted = [...projects].sort(
      (a, b) =>
        safeParseDate(a.dateCreated) - safeParseDate(b.dateCreated)
    );
    const lanes = [];
    return sorted.map((p) => {
...
const rangeMap = useMemo(() => {
  const map = {};
  if (!projectsWithLanes || projectsWithLanes.length === 0) return map;
    ...
    return (
      ...
      >
        <div className="tile-dots">
          ...
          {overflowCount > 0 && (
            <span className="event-overflow">+{overflowCount}</span>
          )}
        </div>
        ...
        <div className="timeline-bars">
          {Array.from({ length: maxLane + 1 }).map((_, laneIdx) => {
            ...
            const hasPrev = prevProjects.some(
              (p) => p.projectId === proj.projectId && p.lane === laneIdx
            );
                    ...
                    <div className="tooltip-header">
                      ...
                      {item.finishline && (
                        <span className="tooltip-date">
                          {new Date(item.finishline).toLocaleDateString()}
                        </span>
                      )}
                    </div>
                    {events.map((ev, idx) => (
