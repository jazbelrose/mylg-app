...
const Messages = ({ initialUserSlug = null }) => {
  ...
  const messages = useMemo(() => {
    ...
    return dedupeById(filtered.map((m) => ({ ...m, read: true }))).sort(
...
const openConversation = async (conversationId) => {
    ...
    setDmThreads((prev) =>
      prev.map((t) =>
        t.conversationId === conversationId ? { ...t, read: true } : t
      )
    );
  ...
  const filteredDMUsers = allUsers.filter(user => {
    ...
    return (
      (userData.collaborators && userData.collaborators.includes(user.userId)) ||
...
/**
  ...
  const handleFileUpload = async (conversationId, file) => {
    ...
    try {
      ...
      const fileUrl =
        `${S3_PUBLIC_BASE}/dms/${encodedConversationId}/${folderKey}/${encodedFileName}`;
  ...
  if (selectedConversation) {
    ...
    if (dmUser) {
      ...
      chatIcon = dmUser.profilePicture ? (
        ...
        <User style={{ width: "40px", height: "40px", opacity: 0.5 }} />
      );
    ...
    >
      ...
      {(!isMobile || !showConversation) && (
        ...
        <div className="sidebar-section">
          ...
          <div style={{ maxHeight: isMobile ? "calc(100vh - 150px)" : "400px", overflowY: "auto" }}>
            <ul style={{ listStyle: "none", padding: 0, margin: 0 }}>
              {dmConversations.map((conv) => (
               ...
               onClick={() => openConversation(conv.id)}
                ...
                >
                  <div className="avatar-wrapper" style={{ marginRight: "8px" }}>
                    <>
                      {conv.profilePicture ? (
                        <img
                        ...
                        <User style={{ width: "32px", height: "32px", opacity: 0.5 }} />
                      )}
      ...
      >
        ...
        <div
          ...
          style={{ display: "flex", justifyContent: "space-between", alignItems: "center", paddingBottom: "5px", marginBottom: "10px" }}
        >
          <h2 style={{ fontSize: "18px" }}>{chatTitle}</h2>
          {chatIcon}
        </div>
        ...
        <div style={{ display: "flex", gap: "10px", alignItems: "center", position: "relative" }}>
          <input
            ...
            onFocus={() => handleMarkRead(selectedConversation)}
            onKeyDown={(e) => {
              if (e.key === "Enter") {
                e.preventDefault();
                sendMessage();
