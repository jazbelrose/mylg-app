...
import React, { useState, useEffect, useRef, forwardRef, useImperativeHandle, useCallback, useMemo, useLayoutEffect, } from 'react';
...
const FileManagerComponent = forwardRef(({ folder = 'uploads', displayName, style, showTrigger = true, isOpen, onRequestClose, }, ref) => {
    ...
    const getFolderIcon = (folderKey, size = 24) => {
        switch (folderKey) {
            case 'uploads':
                return _jsx(Upload, { size: size });
            case 'invoices':
                return _jsx(FileText, { size: size });
            case 'downloads':
                return _jsx(Download, { size: size });
            case 'floorplans':
                return _jsx(Layout, { size: size });
            default:
    ...
    const getFileKind = (fileName) => {
        ...
        if (['xls', 'xlsx', 'csv'].includes(ext))
            return 'spreadsheet';
        if (['dwg', 'vwx'].includes(ext))
            return 'drawing';
        if (['c4d', 'obj'].includes(ext))
            return 'model';
        if (['ai', 'afdesign'].includes(ext))
            return 'adobe';
        return ext;
    };
    ...
    const sortFiles = useCallback((files) => {
    ...
    const displayedFiles = useMemo(() => {
        const filtered = selectedFiles.filter((f) => f.fileName.toLowerCase().includes(searchTerm.toLowerCase()) &&
    ...
    const handleNextImage = () => {
        if (!displayedFiles.length || currentIndex === null)
            return;
    ...
    const removeReferences = async (urls, messagesList) => {
        ...
        for (const url of urls) {
            ...
            const referencing = messagesList.filter((m) => (m.text && m.text.includes(url)) ||
                ...
                if (msg.messageId) {
                    try {
                        ...
                        setProjectMessages((prev) => {
                            ...
                            return {
                                ...prev,
                                [activeProject.projectId]: msgs.map((m) => m.messageId === msg.messageId ? { ...m, text: 'File deleted.', file: undefined } : m),
                            };
                        });
                    }
                    catch (err) {
    ...
    const uploadFiles = async (files) => {
        ...
        await Promise.all(files.map((file, idx) => handleFileUpload(activeProject.projectId, file)
            .then((info) => {
    ...
    const handleFileUpload = (projectId, file) => uploadQueue(async () => {
    ...
    const updateFolderFiles = useCallback((projectId, updatedFiles) => editQueue(async () => {
    ...
    const scheduleFolderUpdate = useCallback((filesPayload) => {
        if (folderKey === 'uploads')
            return;
        ...
        updateTimeoutRef.current = setTimeout(() => {
            ...
            if (pending) {
                updateFolderFiles(activeProject.projectId, pending).catch((err) => console.error('Error updating files:', err));
            }
        }, 500);
    ...
    const renderFilePreview = (file) => {
        ...
        if (['jpg', 'jpeg', 'png'].includes(extension)) {
            ...
            return (_jsx("img", { src: thumbUrl, alt: file.fileName, className: styles.previewImage, onError: (e) => { e.target.src = file.url; }, loading: "lazy" }));
        }
        else if (extension === 'pdf') {
            return _jsx(FaFilePdf, { size: 50, color: "red" });
        }
        else if (extension === 'svg') {
            return _jsx(SiSvg, { size: 50, color: "purple" });
        }
        else if (extension === 'html') {
            return _jsx(SiHtml5, { size: 50, color: "orange" });
        }
        else if (extension === 'txt') {
            return _jsx(FaFileAlt, { size: 50, color: "gray" });
        }
        else if (['xls', 'xlsx', 'csv'].includes(extension)) {
            return _jsx(FaFileExcel, { size: 50, color: "green" });
        }
        else if (['dwg', 'vwx'].includes(extension)) {
            return _jsx(FaDraftingCompass, { size: 50, color: "brown" });
        }
        else if (['c4d', 'obj'].includes(extension)) {
            return _jsx(FaCube, { size: 50, color: "purple" });
        }
        else if (['ai', 'afdesign'].includes(extension)) {
            return _jsx(SiAdobe, { size: 50, color: "orange" });
        }
        else {
            return _jsx(FaFileAlt, { size: 50, color: "blue" });
        }
    };
    ...
    const fetchS3Files = async () => {
        if (!activeProject?.projectId)
            return [];
        const prefixes = folderKey === 'uploads'
            ? ['uploads/', 'lexical/', 'chat_uploads/'].map((dir) => `projects/${activeProject.projectId}/${dir}`)
    ...
    return (_jsxs(_Fragment, { children: [showTrigger && (_jsxs("div", { className: `dashboard-item files files-shared-style ${styles.fileManager}`, onClick: openFilesModal, style: style, children: [_jsxs("div", { className: styles.fileManagerInner, children: [_jsx("span", { className: styles.icon, children: getFolderIcon(folderKey) }), _jsx("span", { children: renderedName })] }), _jsx("span", { className: styles.arrow, children: ">" })] })), _jsxs(Modal, { isOpen: isFilesModalOpen, onRequestClose: closeFilesModal, contentLabel: "Files Modal", shouldCloseOnOverlayClick: !isConfirmingDelete, style: {
                    ...
                    beforeClose: styles.fileModalOverlayBeforeClose,
                }, closeTimeoutMS: 300, children: [_jsxs("div", { className: styles.modalHeader, children: [_jsx("div", { className: styles.folderTabs, children: systemFolders.map((f) => (_jsxs("button", { className: `${styles.tabButton} ${folderKey === f.key ? styles.activeTab : ''}`, onClick: () => setFolderKey(f.key), children: [getFolderIcon(f.key, 16), " ", f.name] }, f.key))) }), _jsxs("div", { className: styles.actions, children: [_jsx("input", { type: "text", placeholder: "Search", value: searchTerm, onChange: (e) => setSearchTerm(e.target.value), className: styles.searchInput }), _jsx(Dropdown, { label: "Filter files", options: filterOptionsList, value: filterOption, onChange: setFilterOption }), _jsx(Dropdown, { label: "Sort files", options: sortOptionsList, value: sortOption, onChange: setSortOption }), _jsx("button", { className: styles.iconButton, onClick: toggleViewMode, "aria-label": "Toggle view", children: _jsx(FontAwesomeIcon, { icon: layoutIcon }) }), _jsx("button", { className: styles.iconButton, onClick: closeFilesModal, "aria-label": "Close", children: _jsx(FontAwesomeIcon, { icon: faXmark }) })] })] }), _jsxs("div", { ref: scrollerRef, className: `${styles.modalContentInner} ${isDragging ? styles.dragging : ''}`, onDragOver: handleDragOver, onDragLeave: handleDragLeave, onDrop: handleDrop, children: [isDragging && (_jsx("div", { className: styles.dragOverlay, children: "Drop files to upload" })), isLoading && (_jsxs("div", { className: styles.loadingOverlay, children: [_jsx(Spinner, { style: { position: 'static' } }), _jsx("div", { className: styles.loadingMessage, children: "Loading files\u2026" })] })), displayedFiles.length === 0 && !isLoading ? (_jsxs("div", { className: styles.emptyMessage, children: [_jsx(Upload, { size: 48 }), _jsx("span", { children: "No files yet." })] })) : (_jsxs(_Fragment, { children: [isSelectMode && (_jsxs("div", { children: [_jsx("input", { type: "checkbox", checked: selectedItems.size === selectedFiles.length, onChange: handleSelectAll }), ' ', "Select All"] })), viewMode === 'grid' ? (_jsx("ul", { className: styles.fileGrid, children: displayedFiles.map((file, index) => (_jsxs("li", { className: styles.fileItem, children: [_jsxs("div", { onClick: () => {
                                                        if (isSelectMode) {
                                                            handleSelectionChange(file.url);
                                                        }
                                                        else {
                                                    ...
                                                    }, className: `${styles.filePreview} ${isSelectMode ? styles.clickable : ''}`, children: [renderFilePreview(file), isSelectMode && (_jsx("div", { className: `${styles.selectionOverlay} ${isSelected(file.url) ? styles.selected : ''}`, children: isSelected(file.url) && (_jsx(FontAwesomeIcon, { icon: faCheck, className: styles.checkIcon })) }))] }), _jsx("div", { className: styles.fileName, title: file.fileName, children: truncateFileName(file.fileName) })] }, file.url))) })) : (_jsxs("table", { className: styles.fileTable, children: [_jsx("thead", { children: _jsxs("tr", { children: [_jsx("th", { children: "Name" }), _jsx("th", { children: "Actions" })] }) }), _jsx("tbody", { children: displayedFiles.map((file, index) => (_jsxs("tr", { children: [_jsx("td", { onClick: () => handleFileClick(file, index), children: file.fileName }), _jsxs("td", { children: [_jsx("button", { className: styles.iconButton, onClick: () => handleDownloadSingle(file.url), "aria-label": "Download file", children: _jsx(FontAwesomeIcon, { icon: faDownload }) }), canDelete && (_jsx("button", { className: styles.iconButton, onClick: () => handleDeleteSingle(file.url), "aria-label": "Delete file", children: _jsx(FontAwesomeIcon, { icon: faTrash }) }))] })] }, file.url))) })] }))] }))] }), _jsx("div", { className: styles.modalFooter, children: selectedFiles.length === 0 ? (_jsx(_Fragment, { children: canUpload && (_jsxs(_Fragment, { children: [_jsx("input", { type: "file", multiple: true, onChange: handleFileSelect, ref: fileInputRef, className: styles.hiddenInput }), _jsxs("button", { className: styles.iconButton, onClick: () => fileInputRef.current.click(), "aria-label": "Upload files", children: [_jsx(FontAwesomeIcon, { icon: faUpload }), " Upload"] })] })) })) : isSelectMode ? (_jsxs(_Fragment, { children: [_jsx("button", { className: styles.iconButton, onClick: handleBulkDownload, "aria-label": "Download selected", children: _jsx(FontAwesomeIcon, { icon: faDownload }) }), canDelete && (_jsx("button", { className: styles.iconButton, onClick: handleDelete, "aria-label": "Delete selected", children: _jsx(FontAwesomeIcon, { icon: faTrash }) })), _jsx("button", { className: styles.iconButton, onClick: () => setIsSelectMode(false), "aria-label": "Cancel selection", children: _jsx(FontAwesomeIcon, { icon: faXmark }) })] })) : (_jsxs(_Fragment, { children: [selectedFiles.length > 0 && (_jsxs("button", { className: styles.iconButton, onClick: toggleSelectMode, "aria-label": "Select files", children: [_jsx(FontAwesomeIcon, { icon: faCheck }), " Select"] })), canUpload && (_jsxs(_Fragment, { children: [_jsx("input", { type: "file", multiple: true, onChange: handleFileSelect, ref: fileInputRef, className: styles.hiddenInput }), _jsxs("button", { className: styles.iconButton, onClick: () => fileInputRef.current.click(), "aria-label": "Upload files", children: [_jsx(FontAwesomeIcon, { icon: faUpload }), " Upload"] })] }))] })) })] }), _jsx(ConfirmModal, { isOpen: isConfirmingDelete, onRequestClose: () => setIsConfirmingDelete(false), onConfirm: performDelete, message: "Are you sure you want to delete the selected files?", className: {
                ...
                } }), _jsx(Modal, { isOpen: isImageModalOpen, onRequestClose: closeImageModal, contentLabel: "Image Preview Modal", className: {
                ...
                }, closeTimeoutMS: 300, children: selectedImage && displayedFiles[currentIndex] && (_jsxs("div", { className: styles.imageWrapper, onTouchStart: handleTouchStart, onTouchMove: handleTouchMove, onTouchEnd: handleTouchEnd, children: [(() => {
                            const ext = displayedFiles[currentIndex].fileName
                                .split('.')
                                .pop()
                                .toLowerCase();
                            if (['jpg', 'jpeg', 'png'].includes(ext)) {
                                return (_jsx("img", { src: selectedImage, alt: "Selected", onError: (e) => {
                                        e.target.src = selectedImage;
                                    }, className: styles.fullImage }));
                            }
                            if (ext === 'pdf') {
                                return (_jsx(PDFPreview, { url: selectedImage, className: styles.pdfPreview, title: displayedFiles[currentIndex].fileName }));
                            }
                            return (_jsxs("div", { className: styles.filePlaceholder, children: [_jsx("div", { className: styles.placeholderIcon, children: renderFilePreview(displayedFiles[currentIndex]) }), _jsx("div", { className: styles.imageInfo, children: displayedFiles[currentIndex].fileName })] }));
                        })(), _jsxs("div", { className: styles.imageTopBar, children: [_jsx("button", { onClick: closeImageModal, className: styles.iconButton, "aria-label": "Close image", children: _jsx(FontAwesomeIcon, { icon: faXmark }) }), _jsx("a", { href: selectedImage, download: true, className: styles.iconButton, "aria-label": "Download image", children: _jsx(FontAwesomeIcon, { icon: faDownload }) })] })] })) })] }));
});
