...
const DesignerComponent = forwardRef<DesignerRef, DesignerComponentProps>(
  (props, ref) => {
    ...
    const saveCanvas = useCallback(
      async (showToast = false) => {
        ...
        try {
          ...
          setActiveProject((prev: any) => (prev ? { ...prev, canvasJson } : prev));
          setIsDirty(false);
          if (showToast) notify("success", "Canvas saved successfully");
        } catch (err: any) {
          console.error("Failed to save canvas:", err);
          if (showToast) notify("error", `Failed to save canvas: ${err.message}`);
        }
      },
    ...
    const handleClear = useCallback(() => {
      ...
      fabricCanvas.getObjects().forEach((obj: any) => fabricCanvas.remove(obj));
      ...
      const loadCanvas = async () => {
        ...
        try {
          ...
          if (jsonString) {
            ...
            if (jsonObj && Array.isArray(jsonObj.objects)) {
              isRestoringHistory.current = true;
    ...
    const handleMouseDown = (e: any) => {
      ...
      if (mode === TOOL_MODES.RECT) {
        ...
        (fabricCanvas as any).__drawingObject = rect;
        (fabricCanvas as any).__isDrawingRect = true;
    ...
    const handleMouseMove = (e: any) => {
      const fabricCanvas = fabricCanvasRef.current;
      if (!fabricCanvas || !(fabricCanvas as any).__drawingObject) return;

      const pointer = fabricCanvas.getPointer(e);
      const obj = (fabricCanvas as any).__drawingObject as any;
    ...
    const handleMouseUp = () => {
      ...
      if ((fabricCanvas as any).__isDrawingRect && (fabricCanvas as any).__drawingObject) {
        fabricCanvas.setActiveObject((fabricCanvas as any).__drawingObject);
        (fabricCanvas as any).__drawingObject = null;
        (fabricCanvas as any).__isDrawingRect = false;
        saveHistory();
        updateObjects();
        changeMode(TOOL_MODES.SELECT);
      }
    ...
    const handleCopy = useCallback(async () => {
      ...
      const active = fabricCanvas.getActiveObject();
      if (active) {
        clipboard.current = await (active as any).clone();
      }
    }, []);
    ...
    const handlePaste = useCallback(async () => {
      ...
      const clonedObj = await (clipboard.current as any).clone();
      fabricCanvas.discardActiveObject();
      clonedObj.set({
    ...
    const renameObject = (obj: any, name: string) => {
      obj.name = name;
      updateObjects();
      markDirty();
    };

    const selectLayer = (obj: any, id: string | number) => {
      const fabricCanvas = fabricCanvasRef.current;
      if (!fabricCanvas) return;
      ...
      <div style={{ display: "flex", height: "100%" }}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
          <div
            ...
            onMouseDown={mode === TOOL_MODES.RECT ? (handleMouseDown as any) : undefined}
            onMouseMove={mode === TOOL_MODES.RECT ? (handleMouseMove as any) : undefined}
