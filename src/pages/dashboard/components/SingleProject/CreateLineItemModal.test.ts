...
test('autosaves after field change and reuses returned id', async () => {
    ...
    render(_jsx(CreateLineItemModal, { isOpen: true, onRequestClose: () => { }, onSubmit: onSubmit }));
...
test('closing with unsaved changes prompts to save', async () => {
    ...
    const user = userEvent.setup({ advanceTimers: jest.advanceTimersByTime });
    render(_jsx(CreateLineItemModal, { isOpen: true, onRequestClose: onRequestClose, onSubmit: onSubmit, initialData: { description: 'Initial' } }));
    const desc = screen.getByLabelText('Description');
    await user.clear(desc);
    await user.type(desc, 'Changed');
    await user.click(screen.getByText('Cancel'));
    expect(await screen.findByText('You have unsaved changes, do you want to save this line item?')).toBeInTheDocument();
