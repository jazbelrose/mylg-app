...
jest.mock('./PDFPreview.js', () => ({ title }) => _jsx("canvas", { title: title }));
jest.mock('../../../../utils/api', () => ({
    API_BASE_URL: 'base',
    ZIP_FILES_URL: 'zip',
...
const FileManagerComponent = require('./FileManager.js').default;
const { NotificationContainer } = require('../../../../components/ToastNotifications.js');
beforeEach(() => {
...
test.skip('deleting a file via toast confirmation updates the list without closing modal', async () => {
    render(_jsxs(_Fragment, { children: [_jsx(NotificationContainer, {}), _jsx(FileManagerComponent, { folder: "invoices" })] }));
