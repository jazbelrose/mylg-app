...
const ChatPanel = ({ projectId, initialFloating = false, onFloatingChange, initialOpen = true }) => {
    ...
    return (_jsxs("div", { ref: panelRef, className: `chat-panel ${open ? 'open' : 'closed'} ${floating ? 'floating' : 'docked'} ${isMobile ? 'bottom' : ''}`, style: panelStyle, onClick: () => {
            ...
            if (!open)
                handleSetOpen(true);
        }, children: [_jsx(ProjectMessagesThread, { projectId: projectId, open: open, setOpen: handleSetOpen, floating: floating, setFloating: handleSetFloating, startDrag: startDrag }), floating && !isMobile && (_jsx("div", { className: "chat-panel-resizer", onMouseDown: startResize }))] }));
};
