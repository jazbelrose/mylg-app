...
const renderFilePreview = (file, folderKey = "chat_uploads") => {
    ...
    if (["jpg", "jpeg", "png"].includes(extension)) {
        ...
        return (_jsxs("div", { style: commonStyle, children: [_jsx(FaFilePdf, { size: 50, color: "red" }), _jsx("span", { style: fileNameStyle, children: file.fileName })] }));
    }
    else if (extension === "svg") {
        return (_jsxs("div", { style: commonStyle, children: [_jsx(SiSvg, { size: 50, color: "purple" }), _jsx("span", { style: fileNameStyle, children: file.fileName })] }));
    }
    else if (extension === "txt") {
        return (_jsxs("div", { style: commonStyle, children: [_jsx(FaFileAlt, { size: 50, color: "gray" }), _jsx("span", { style: fileNameStyle, children: file.fileName })] }));
    }
    else if (["xls", "xlsx", "csv"].includes(extension)) {
        return (_jsxs("div", { style: commonStyle, children: [_jsx(FaFileExcel, { size: 50, color: "green" }), _jsx("span", { style: fileNameStyle, children: file.fileName })] }));
    }
    else if (["dwg", "vwx"].includes(extension)) {
        return (_jsxs("div", { style: commonStyle, children: [_jsx(FaDraftingCompass, { size: 50, color: "brown" }), _jsx("span", { style: fileNameStyle, children: file.fileName })] }));
    }
    else if (["c4d", "obj"].includes(extension)) {
        return (_jsxs("div", { style: commonStyle, children: [_jsx(FaCube, { size: 50, color: "purple" }), _jsx("span", { style: fileNameStyle, children: file.fileName })] }));
    }
    else if (extension === "ai") {
        return (_jsxs("div", { style: commonStyle, children: [_jsx(SiAdobe, { size: 50, color: "orange" }), _jsx("span", { style: fileNameStyle, children: file.fileName })] }));
    }
    else if (extension === "afdesign") {
        return (_jsxs("div", { style: commonStyle, children: [_jsx(SiAffinitydesigner, { size: 50, color: "orange" }), _jsx("span", { style: fileNameStyle, children: file.fileName })] }));
    }
    else if (extension === "afpub") {
        return (_jsxs("div", { style: commonStyle, children: [_jsx(SiAffinitypublisher, { size: 50, color: "green" }), _jsx("span", { style: fileNameStyle, children: file.fileName })] }));
    }
    else if (extension === "js") {
        return (_jsxs("div", { style: commonStyle, children: [_jsx(FaFileAlt, { size: 50, color: "blue", style: { fill: "blue" } }), _jsx("span", { style: fileNameStyle, children: file.fileName })] }));
    }
...
const ProjectMessagesThread = ({ projectId, open, setOpen, floating, setFloating, startDrag, headerOffset = 0 }) => {
    ...
    const messages = useMemo(() => {
        ...
        const filtered = all.filter((m) => !(deletedMessageIds.has(m.messageId) ||
                ...
                if (data.action === 'deleteMessage' &&
                    ...
                    if (pid === projectId) {
                        ...
                        setProjectMessages((prev) => {
                            ...
                            const updated = msgs.filter((m) => !((data.messageId && m.messageId === data.messageId) ||
        ...
        const fetchMessages = async () => {
            ...
            for (let attempt = 0; attempt <= maxRetries; attempt++) {
                try {
                    ...
                    const res = await apiFetch(`${GET_PROJECT_MESSAGES_URL}?projectId=${projectId}`, attempt > 0 ? { skipRateLimit: true } : undefined);
                    const data = await res.json();
                    if (Array.isArray(data)) {
                        const filtered = data.filter((m) => !(deletedMessageIds.has(m.messageId) ||
                            deletedMessageIds.has(m.optimisticId)));
                        setProjectMessages(prev => ({
                            ...prev,
                    ...
                    setProjectMessages(prev => ({ ...prev, [projectId]: [] }));
                    setWithTTL(pmKey(projectId), []);
                    setErrorMessage(error.message.includes("Rate limit")
    ...
    const handleDrop = async (event) => {
        ...
        for (const file of files) {
            ...
            try {
                ...
                setProjectMessages(prev => {
                    const msgs = Array.isArray(prev[projectId]) ? prev[projectId] : [];
                    const updated = msgs.map(msg => msg.optimisticId === optimisticId
                        ? {
                            ...msg,
    ...
    const editMessage = async (message, newText) => {
        if (!message.messageId || !newText)
            return;
        try {
            const res = await apiFetch(`${EDIT_MESSAGE_URL}/project/${encodeURIComponent(message.messageId)}`, {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    content: newText,
            ...
            setProjectMessages(prev => {
                const msgs = Array.isArray(prev[projectId]) ? prev[projectId] : [];
                const updated = msgs.map(m => m.messageId === message.messageId
    ...
    const reactToMessage = (messageId, emoji) => {
        if (!messageId || !emoji)
            return;
        toggleReaction(messageId, emoji, userData.userId, `project#${projectId}`, 'project', ws);
    };
    return (_jsxs(_Fragment, { children: [_jsxs("div", { className: `project-messages ${isDragging ? "dragging" : ""} ${!open ? "closed" : ""} ${floating ? "floating" : ""}`, style: {
                    display: "flex",
                    flexDirection: "column",
                    width: "100%",
                    ...
                    setIsDragging(false);
                    handleDrop(e);
                }, children: [isLoading && _jsx(SpinnerOverlay, {}), errorMessage && _jsx("div", { className: "error-message", children: errorMessage }), _jsxs("div", { className: "thread-panel-header chat-panel-header", onMouseDown: startDrag, children: [_jsx("h3", { className: "project-thread-title", children: `# ${activeProject?.title || projectId} Thread` }), _jsxs("div", { children: [floating && (_jsx("button", { className: "icon-btn", onClick: () => setOpen(o => !o), "aria-label": open ? 'Collapse' : 'Expand', title: open ? 'Collapse' : 'Expand', children: open ? _jsx(ChevronDown, { size: 16 }) : _jsx(ChevronUp, { size: 16 }) })), _jsx("button", { className: "icon-btn", onClick: () => setFloating(f => !f), "aria-label": floating ? 'Dock' : 'Float', title: floating ? 'Dock' : 'Float', children: floating ? _jsx(Dock, { size: 16 }) : _jsx(Move, { size: 16 }) })] })] }), open && (_jsxs("div", { className: `chat-messages`, ref: messagesContainerRef, style: {
                            flexGrow: 1,
                            overflowY: "auto",
                            padding: "10px",
                            borderRadius: "5px",
                        ...
                        }, children: [messages.length === 0 && !isLoading ? (_jsx("div", { style: { color: "#aaa", fontSize: "16px", textAlign: "center" }, children: "No messages yet." })) : (messages.map((msg, index) => (_jsx(MessageItem, { msg: msg, prevMsg: messages[index - 1], userData: userData, allUsers: allUsers, openPreviewModal: openPreviewModal, folderKey: folderKey, renderFilePreview: renderFilePreview, getFileNameFromUrl: getFileNameFromUrl, onDelete: (m) => setDeleteTarget(m), onEditRequest: (m) => setEditTarget(m), onReact: reactToMessage }, msg.messageId || msg.optimisticId || msg.timestamp)))), messages.length > 0 && _jsx("div", {})] })), open && (_jsxs("div", { className: "message-input-container", children: [_jsx("input", { type: "text", placeholder: "Type a message or drag files here...", value: newMessage, onChange: (e) => setNewMessage(e.target.value), onKeyDown: (e) => {
                                    if (e.key === "Enter") {
                                        e.preventDefault();
                                        sendMessage();
                                    }
                                }, className: "message-input" }), _jsx("button", { onClick: sendMessage, className: "send-button", children: "Send" })] })), sendError && _jsx("div", { className: "error-message", children: sendError }), isDragging && (_jsx("div", { className: "drag-overlay", children: "Drop files to upload" }))] }), _jsx(Modal, { isOpen: isPreviewModalOpen, onRequestClose: closePreviewModal, contentLabel: "File Preview Modal", className: "messages-modal-content", overlayClassName: "messages-modal-overlay", children: selectedPreviewFile && (_jsxs("div", { className: "preview-container", children: [(() => {
                            const ext = selectedPreviewFile.fileName
                                .split(".")
                                .pop()
                                .toLowerCase();
                            if (["jpg", "jpeg", "png"].includes(ext)) {
                                return (_jsx("img", { src: selectedPreviewFile.finalUrl || selectedPreviewFile.url, alt: selectedPreviewFile.fileName, className: "preview-image" }));
                            }
                            else if (ext === "pdf") {
                                return (_jsx(PDFPreview, { url: selectedPreviewFile.finalUrl || selectedPreviewFile.url, className: "preview-pdf", title: selectedPreviewFile.fileName }));
                            }
                            else {
