  ...
  const handleChange = (e) => {
    ...
    setItem((prev) => {
      ...
      if (
        ...
        if (name === 'itemBudgetedCost') budgeted = parseBudget(value);
        if (name === 'itemActualCost') actual = parseBudget(value);
        if (name === 'itemReconciledCost') reconciled = parseBudget(value);

        if (
          (name === 'itemActualCost' || name === 'itemReconciledCost') &&
          prevFinal
        ) {
        ...
        {fields.map((f) => (
          <React.Fragment key={f.name}>
              ...
              className={`${styles.field} ${
                TOOLTIP_TEXT[f.name] ? styles.tooltipLabel : ''
              }`}
              title={TOOLTIP_TEXT[f.name] || undefined}
            >
              <span>{f.label}</span>
              {f.type === 'select' ? (
                f.name === 'paymentStatus' ? (
                  <span className={styles.paymentStatusContainer}>
                    <select
                      ...
                      disabled={f.name === 'elementKey' || f.name === 'elementId'}
                    >
                      <option hidden value="" />
                      {f.options.map((o) => (
                ...
                <input
                  ...
                  value={item[f.name]}
                  onChange={handleChange}
                  onBlur={['currency', 'percent'].includes(f.type) ? handleBlur : undefined}
            ...
            {['description', 'itemFinalCost', 'paymentStatus', 'endDate', 'invoiceGroup', 'vendorInvoiceNumber'].includes(f.name) && (
