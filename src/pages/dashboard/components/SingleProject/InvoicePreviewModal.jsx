...
const InvoicePreviewModal = ({
  ...
  const loadInvoice = async (url) => {
    try {
      ...
      if (!page) return;
      setBrandLogoUrl(
        page.querySelector('.invoice-header img')?.getAttribute('src') || ''
      );
      setLogoDataUrl(null);
      ...
      const infoSpans = page.querySelectorAll(
        '.billing-info > div:last-child span'
      );
        ...
        const candidate = groupFields
          ...
          .find((field) => {
            const opts = Array.from(
              new Set(items.map((it) => (it[field] || '').trim()).filter(Boolean))
            );
  ...
  useEffect(() => {
    const vals = Array.from(
      new Set(items.map((it) => (it[groupField] || "").trim()).filter(Boolean))
    );
  ...
  const subtotal = filtered.reduce(
    (sum, it) => sum + (parseFloat(it.itemFinalCost) || 0),
    0
  );
  ...
  const buildInvoiceHtml = () => {
    ...
    const htmlPages = pageIndexes
      .map((idx) => {
        ...
        const rowsHtml = pageRows
          .map((row) =>
            row.type === "group"
              ...
              : `<tr><td>${row.item.description || ""}</td><td>${
                  row.item.quantity || ""
                }</td><td>${row.item.unit || ""}</td><td>${formatCurrency(
                  (parseFloat(row.item.itemFinalCost) || 0) / (parseFloat(row.item.quantity) || 1)
                )}</td><td>${formatCurrency(parseFloat(row.item.itemFinalCost) || 0)}</td></tr>`
          )
          .join("");
        ...
        const totalsHtml =
          idx === pages.length - 1
            ? `<div class="bottom-block"><div class="totals"><div>Subtotal: <span>${formatCurrency(
                subtotal
              )}</span></div><div>Deposit received: <span>${deposit}</span></div><div><strong>Total Due: <span>${total}</span></strong></div></div><div class="notes">${notesText}</div><div class="footer">${headerName}</div></div>`
            : "";
    ...
    >
      ...
      <div className={styles.currentFileRow}>
        ...
        <div className={styles.buttonGroup}>
          ...
          <button className={styles.iconButton} onClick={exportHtml} aria-label="Download HTML">
        ...
        >
            ...
            <table className="items-table">
              ...
              <tbody>
                {rowsData.map((row, idx) =>
                  row.type === 'group' ? (
                    <tr key={idx} className="group-header">
                      <td colSpan="5">{row.group}</td>
                    </tr>
                  ) : (
                    <tr key={row.item.budgetItemId}>
                      <td>{row.item.description || ''}</td>
                      <td>{row.item.quantity || ''}</td>
                      <td>{row.item.unit || ''}</td>
                      <td>{formatCurrency((parseFloat(row.item.itemFinalCost) || 0) / (parseFloat(row.item.quantity) || 1))}</td>
                      <td>{formatCurrency(parseFloat(row.item.itemFinalCost) || 0)}</td>
                    </tr>
                  )
                )}
              </tbody>
          ...
          <div className="bottom-block">
            <div className="totals">
              <div>Subtotal: <span contentEditable suppressContentEditableWarning>{formatCurrency(subtotal)}</span></div>
              <div>
                Deposit received:
                <span
                  contentEditable
                  ...
                  onBlur={(e) => {
                    setDepositReceived(
                      parseFloat(e.currentTarget.textContent.replace(/[$,]/g, "")) || 0
                    );
                    setInvoiceDirty(true);
                  }}
                >
            ...
            <div className="footer" contentEditable suppressContentEditableWarning>{project?.company || 'Company Name'}</div>
          </div>
        </div>
        {pages[currentPage] && (
          <React.Fragment>
              ...
              <table className="items-table">
                ...
                <tbody>
                  {currentRows.map((row, idx2) =>
                    row.type === 'group' ? (
                      <tr key={`g-${currentPage}-${idx2}`} className="group-header">
                        <td colSpan="5">{row.group}</td>
                      </tr>
                    ) : (
                      <tr key={row.item.budgetItemId}>
