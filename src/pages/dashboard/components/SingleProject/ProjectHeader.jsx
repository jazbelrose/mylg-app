...
const ProjectHeader = ({
  ...
  const endDate = useMemo(
    () => safeParse(localActiveProject?.finishline),
  ...
  const [
    ...
    localActiveProject?.productionStart ||
  ...
  const [invoiceBrandName, setInvoiceBrandName] = useState(
    localActiveProject?.invoiceBrandName || ""
  );
  const [invoiceBrandAddress, setInvoiceBrandAddress] = useState(
    localActiveProject?.invoiceBrandAddress || ""
  );
  const [invoiceBrandPhone, setInvoiceBrandPhone] = useState(
    localActiveProject?.invoiceBrandPhone || ""
  );
  const [clientName, setClientName] = useState(
    localActiveProject?.clientName || ""
  );
  const [clientAddress, setClientAddress] = useState(
    localActiveProject?.clientAddress || ""
  );
  const [clientPhone, setClientPhone] = useState(
    localActiveProject?.clientPhone || ""
  );
  ...
  const [selectedColor, setSelectedColor] = useState(
    localActiveProject?.color || "#FA3356"
  );
      ...
      <div className="project-header">
        <div className="header-content">
          <div className="left-side">
            ...
            />
            {/* Project Thumbnail or Initial (clickable to update thumbnail) */}
            <div
              onClick={openThumbnailModal}
              onKeyDown={(e) => handleKeyDown(e, openThumbnailModal)}
              role="button"
              tabIndex={0}
            ...
            {/* Status SVG - clicking on it opens the edit status modal */}
            <svg
              id="StatusSVG"
              viewBox="0 0 400 400"
              ...
              role="button"
              tabIndex={0}
              title={`Status: ${displayStatus} Complete`}
            ...
            >
              ...
              {localActiveProject && (
                ...
                >
                  {parseStatusToNumber(localActiveProject.status) < 100 && (
                    <animate
                      key={`status-animation-${localActiveProject.status}`}
...
const ProjectTabs = ({ projectSlug }) => {
  ...
  const getActiveIndex = useCallback(() => {
    ...
    if (location.pathname.startsWith(`${base}/designer`)) return 3;
    return 0;
  ...
  return (
    ...
    >
      ...
      <button
        ...
        className={
          location.pathname === `/dashboard/projects/${projectSlug}`
            ? "active"
            : ""
        }
      ...
      {showEditorTab && (
        <button
          ...
          onClick={() =>
            navigate(`/dashboard/projects/${projectSlug}/designer`, {
              state: { fromTab: getActiveIndex() },
            })
          }
          className={
            location.pathname.startsWith(
              `/dashboard/projects/${projectSlug}/designer`
            )
              ? "active"
              : ""
          }
