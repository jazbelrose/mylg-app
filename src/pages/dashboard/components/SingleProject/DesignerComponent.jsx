...
if (!StaticCanvas.prototype._defensivePatched) {
  ...
  StaticCanvas.prototype.getContext = function() {
    ...
    if (!this.lowerCanvasEl || typeof this.lowerCanvasEl.getContext !== 'function') return undefined;
    ...
    const handleBeforeUnload = (e) => {
      ...
      if (fabricCanvasRef.current && activeProject?.projectId) {
        ...
        navigator.sendBeacon(
          `${API_BASE_URL}/editProject?projectId=${activeProject.projectId}`,
...
useEffect(() => {
  ...
  const handleMouseMove = (e) => {
    const fabricCanvas = fabricCanvasRef.current;
    if (!fabricCanvas || !fabricCanvas.__drawingObject) return;
    const pointer = fabricCanvas.getPointer(e);
    ...
    <div style={{ display: 'flex', height: '100%' }}>
      <div className={styles.layersPanel}>
        ...
        {objects.map(({ id, name, obj }) => (
          <div
            key={id}
            className={`${styles.layerItem} ${selectedId === id ? styles.layerItemSelected : ''}`}
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
        {/* UnifiedToolbar now controls the canvas, so local toolbar is removed */}
        <div
          ref={containerRef}
        ...
        >
          ...
          {/* Canvas element is created dynamically in useLayoutEffect */}
        </div>
