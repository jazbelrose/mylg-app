...
const InvoicePreviewModal: React.FC<InvoicePreviewModalProps> = ({
  ...
  const fetchInvoiceFiles = async (): Promise<SavedInvoice[]> => {
    if (!project?.projectId) return [];
    const prefix = `projects/${project.projectId}/invoices/`;
    try {
      const res = await list({ prefix, options: { accessLevel: "public" as any } });
      return (res.items || [])
        ...
        const candidate = (groupFields.map((g) => g.value) as GroupField[]).find(
          (field) => {
            const opts = Array.from(
              new Set(
                items
                  .map((it) => (String((it as any)[field] || "")).trim())
                  .filter(Boolean)
              )
            );
  ...
  useEffect(() => {
    ...
    setBrandName(userData?.brandName || (userData as any)?.company || "");
  ...
  useEffect(() => {
    ...
    if ((revision as any)?.revision != null) {
      setCurrentFileName(`invoice-revision-${(revision as any).revision}.html`);
    } else {
  ...
  useEffect(() => {
    const dirty =
      ...
      (brandName || "") !== (userData?.brandName || (userData as any)?.company || "") ||
  ...
  useEffect(() => {
    const vals = Array.from(
      new Set(
        items
          .map((it) => String((it as any)[groupField] || "").trim())
          .filter(Boolean)
      )
    );
  ...
  const filtered = groupValues.length === 0
    ? items
    : items.filter((it) => groupValues.includes(String((it as any)[groupField]).trim()));
  ...
  const rowsData: RowData[] = useMemo(() => {
    ...
    groups.forEach((grp) => {
      ...
      items
        .filter((it) => String((it as any)[groupField]).trim() === grp)
        ...
        const totalsHtml =
          idx === pages.length - 1
            ? `<div class="bottom-block">
                 ...
                 <div class="footer">${headerName}</div>
               </div>`
            : "";
  ...
  const exportHtml = () => {
    ...
    if (!html) return;
    const file =
      (revision as any)?.revision != null
        ? `invoice-revision-${(revision as any).revision}.html`
        : "invoice.html";
  ...
  const saveInvoice = async () => {
    ...
    const rev = (revision as any)?.revision ?? "0";
  ...
  const handleSaveHeader = async () => {
    try {
      ...
      if (logoDataUrl && logoDataUrl.startsWith("data:") && userData?.userId) {
        ...
        await uploadData({ key: filename, data: file, options: { accessLevel: "public" as any } });
