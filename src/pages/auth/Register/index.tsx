...
export function Register(): JSX.Element {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const cognitoSub =
        (signUpResponse as any)?.userId ??
        (signUpResponse as any)?.userSub ??
        (signUpResponse as any)?.user?.userId ??
        (signUpResponse as any)?.user?.sub;
      ...
      if (error?.name === "UsernameExistsException") {
        ...
        const message =
          error?.message || error?.name || "Registration failed";
  ...
  if (isRegistered) {
    return (
      <EmailVerification
        registrationData={registrationDetails as any}
        userEmail={email}
      />
    );
  }
