...
export const SocketProvider = ({ children }) => {
      ...
      socket.onmessage = (event) => {
        try {
          ...
          if (data.action === 'timelineUpdated' && data.projectId && Array.isArray(data.events)) {
            setProjects(prev =>
              prev.map(p =>
                p.projectId === data.projectId ? { ...p, timelineEvents: data.events } : p
              )
            );
            ...
            setActiveProject(prev =>
              prev && prev.projectId === data.projectId ? { ...prev, timelineEvents: data.events } : prev
            );
            return;
          }
          ...
          if (data.action === 'projectUpdated' && data.projectId && data.fields && typeof data.fields === 'object') {
            setProjects(prev =>
              prev.map(p =>
                p.projectId === data.projectId ? { ...p, ...data.fields } : p
              )
            );
            setUserProjects(prev =>
              prev.map(p =>
            ...
            setActiveProject(prev =>
              prev && prev.projectId === data.projectId ? { ...prev, ...data.fields } : prev
            );
            return;
          }
          ...
          if (data.conversationType === 'dm') {
            if (data.action === 'sendMessage' || data.action === 'newMessage') {
              if (
                deletedMessageIds.has(data.messageId) ||
              ...
              setUserData(prev => {
                ...
                return {
                  ...prev,
                  messages: msgs.map(m =>
                    m.messageId === data.messageId ? { ...m, reactions: data.reactions } : m
                  ),
                };
              });
            }
  ...
  return (
    <SocketContext.Provider value={{ ws, isConnected, onlineUsers }}>
      {children}
