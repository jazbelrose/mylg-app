...
import { THREADS_URL, fetchAllUsers, fetchUserProfile as fetchUserProfileApi, fetchProjectsFromApi, fetchProjectById, fetchEvents, updateTimelineEvents as updateTimelineEventsApi, updateProjectFields as updateProjectFieldsApi, updateUserProfile, apiFetch, fetchPendingInvites, sendProjectInvite, acceptProjectInvite, declineProjectInvite, cancelProjectInvite, GET_PROJECT_MESSAGES_URL, } from '../../utils/api';
...
export const DataProvider = ({ children }) => {
    ...
    const toggleReaction = (msgId, emoji, reactorId, conversationId, conversationType, ws) => {
        if (!msgId || !emoji || !reactorId)
            return;
        const updateArr = (arr = []) => arr.map(m => {
    ...
    const fetchProjects = async (retryCount = 0) => {
        const now = Date.now();
        if (now - lastFetchRef.current < 2000 && retryCount === 0)
            return;
        ...
        try {
            ...
            const limit = pLimit(3);
            const withEvents = await Promise.all((Array.isArray(dataItems) ? dataItems : []).map((p) => limit(async () => {
                try {
            ...
            if (isAdmin) {
                ...
                const filteredProjects = withIds.filter((project) => Array.isArray(project.team) &&
    ...
    const updateTimelineEvents = async (projectId, events) => {
        ...
        try {
            ...
            setActiveProject((prev) => prev && prev.projectId === projectId ? { ...prev, timelineEvents: withIds } : prev);
            setProjects((prev) => Array.isArray(prev)
                ? prev.map((p) => p.projectId === projectId ? { ...p, timelineEvents: withIds } : p)
                : prev);
            setUserProjects((prev) => Array.isArray(prev)
                ? prev.map((p) => p.projectId === projectId ? { ...p, timelineEvents: withIds } : p)
                : prev);
        }
        catch (error) {
