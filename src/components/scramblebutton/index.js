...
import "./style.css";
export const ScrambleButton = ({ text, to, className = "", submitMode, fontSize, padding, // ✅ Accept padding as a prop
...props }) => {
    ...
    const handleMouseEnter = () => {
        if (isTouchDevice && buttonType === "submit")
            return;
    ...
    const buttonElem = (_jsx("button", { ref: btnRef, type: buttonType, className: `scramble-button ${className}`, onMouseEnter: handleMouseEnter, onMouseLeave: handleMouseLeave, style: {
            fontSize,
            padding,
            display: "inline-flex",
            alignItems: "center",
            ...
            ...propStyle,
        }, ...restProps, children: _jsx("span", { className: "scrambled", children: text }) }));
    if (to && !submitMode) {
        return _jsx(Link, { to: to, children: buttonElem });
    }
    return buttonElem;
};
