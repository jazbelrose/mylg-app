...
import { $createParagraphNode, $getNodeByKey, $getSelection, $isRangeSelection, COMMAND_PRIORITY_EDITOR, COMMAND_PRIORITY_LOW, createCommand, KEY_ARROW_DOWN_COMMAND, KEY_ARROW_LEFT_COMMAND, KEY_ARROW_RIGHT_COMMAND, KEY_ARROW_UP_COMMAND, KEY_BACKSPACE_COMMAND, KEY_DELETE_COMMAND, } from 'lexical';
...
export function LayoutPlugin() {
    ...
    useEffect(() => {
        ...
        return mergeRegister(editor.registerCommand(KEY_ARROW_DOWN_COMMAND, () => $onEscape(false), COMMAND_PRIORITY_LOW), editor.registerCommand(KEY_ARROW_RIGHT_COMMAND, () => $onEscape(false), COMMAND_PRIORITY_LOW), editor.registerCommand(KEY_ARROW_UP_COMMAND, () => $onEscape(true), COMMAND_PRIORITY_LOW), editor.registerCommand(KEY_ARROW_LEFT_COMMAND, () => $onEscape(true), COMMAND_PRIORITY_LOW), editor.registerCommand(KEY_BACKSPACE_COMMAND, () => {
            const selection = $getSelection();
            ...
            return false;
        }, COMMAND_PRIORITY_LOW), editor.registerCommand(KEY_DELETE_COMMAND, () => {
            const selection = $getSelection();
            if ($isRangeSelection(selection) && selection.isCollapsed()) {
                const anchorNode = selection.anchor.getNode();
                const container = $findMatchingParent(anchorNode, $isLayoutContainerNode);
            ...
            return false;
        }, COMMAND_PRIORITY_LOW), editor.registerCommand(INSERT_LAYOUT_COMMAND, (template) => {
            editor.update(() => {
    ...
    return (_jsxs("div", { style: { position: 'relative', display: 'inline-block' }, children: [_jsx("button", { ref: buttonRef, type: "button", "aria-label": "Insert Layout", onClick: toggleDropdown, className: "layout-toggle-button", children: _jsx(LayoutIcon, { height: 16, color: "#777" }) }), activeDropdown === layoutDropdownId && (_jsxs("div", { ref: dropdownRef, className: "layout-dropdown", children: [_jsx("button", { type: "button", onClick: () => handleInsertLayout('1fr 1fr'), className: "layout-dropdown-item", children: "2 Columns (Equal Width)" }), _jsx("button", { type: "button", onClick: () => handleInsertLayout('25% 75%'), className: "layout-dropdown-item", children: "2 Columns (25% - 75%)" }), _jsx("button", { type: "button", onClick: () => handleInsertLayout('1fr 1fr 1fr'), className: "layout-dropdown-item", children: "3 Columns (Equal Width)" }), _jsx("button", { type: "button", onClick: () => handleInsertLayout('25% 50% 25%'), className: "layout-dropdown-item", children: "3 Columns (25% - 50% - 25%)" }), _jsx("button", { type: "button", onClick: () => handleInsertLayout('1fr 1fr 1fr 1fr'), className: "layout-dropdown-item", children: "4 Columns (Equal Width)" })] }))] }));
}
