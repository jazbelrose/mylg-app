...
import "./marker.css";
const Map = forwardRef(({ location, address, scrollWheelZoom, dragging, touchZoom, showUserLocation, userThumbnail, projectThumbnail, isEditable = false, onLocationChange, otherUsers = [], onUserLocation, }, ref) => {
    const mapRef = useRef(null);
    const mapInstance = useRef(null);
    const userMarkerRef = useRef(null);
    const accuracyCircleRef = useRef(null);
    const projectMarkerRef = useRef(null);
    const otherUsersMarkersRef = useRef({});
    const otherUsersAccuracyRef = useRef({});
    useEffect(() => {
        ...
        const attribution = 'Map tiles by <a href="https://stamen.com">Stamen Design</a>, ' +
            'hosted by <a href="https://stadiamaps.com">Stadia Maps</a> — ' +
            'Map data © <a href="https://openstreetmap.org">OpenStreetMap</a> contributors';
        L.tileLayer(`https://tiles.stadiamaps.com/tiles/stamen_toner/{z}/{x}/{y}{r}.png?api_key=${apiKey}`, {
            attribution,
            maxZoom: 20,
            tileSize: 256,
            zoomOffset: 0,
        }).addTo(mapInstance.current);
        // handle sizing if container was hidden initially
    ...
    useEffect(() => {
        ...
        if (location.lat && location.lng) {
            ...
            const latLngs = [projectMarkerRef.current.getLatLng()];
    ...
    }, [scrollWheelZoom, dragging, touchZoom, location, address, projectThumbnail, isEditable, onLocationChange, otherUsers]);
    useEffect(() => {
        if (!mapInstance.current)
            return;
        if (!showUserLocation) {
        ...
        if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition((position) => {
                ...
                const iconHtml = userThumbnail
                    ...
                    : `<svg width="24" height="24" viewBox="0 0 24 24"><path fill="#ff5722" stroke="white" stroke-width="2" d="M12 2C8.1 3 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7z"/><circle cx="12" cy="9" r="3" fill="white"/></svg>`;
                const iconSize = userThumbnail ? [32, 32] : [24, 24];
                const iconAnchor = userThumbnail ? [16, 16] : [12, 24];
    ...
    }, [showUserLocation, userThumbnail, onUserLocation]);
    useEffect(() => {
        if (!mapInstance.current)
            return;
        ...
        users.forEach((u) => {
            ...
            const iconSize = u.thumbnail ? [32, 32] : [24, 24];
            const iconAnchor = u.thumbnail ? [16, 16] : [12, 24];
    ...
    useEffect(() => {
        if (!mapInstance.current || !isEditable)
            return;
        const handleClick = (e) => {
    ...
    return _jsx("div", { id: "map", style: { height: "100%", width: "100%" }, ref: mapRef });
});
