...
// DynamoDB client (v2) with mild retry/backoff
...
// Simple exponential backoff wrapper for throttling bursts
async function withBackoff(op, retries = 5, delay = 100) {
  try {
    return await op();
  } catch (err) {
    if (
      ...
      return withBackoff(op, retries - 1, delay * 2);
    }
    throw err;
  }
}
...
export const handler = async (event) => {
  ...
  try {
    ...
    return json(status, CORS, { error: 'Internal Server Error', details: err?.message });
  }
};
